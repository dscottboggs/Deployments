...
def setup_reverse_proxy():
    ...
    def dhparam_is_done():
        ...
        letsencrypt = client.containers.list(
            filters={
                "name": "reverse_proxy_letsencrypt-nginx-proxy-companion_1"}
        )[0]
        reverse_proxy = client.containers.list(
            filters={
                "name": "reverse_proxy_nginx-proxy_1"})[0]
        return check(letsencrypt) and check(reverse_proxy)

    while not dhparam_is_done():
        print("Waiting for dhparam.pem to be generated by the containers.")
        sleep(3)
    backup = BackupReverseProxy()
